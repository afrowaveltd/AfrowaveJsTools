...
const fallbackMappings = [
	...
	{ mdStart: '```', htmlElement: 'pre', wrapWith: 'codeblock', langSupport: true }
];
...
export async function convertMarkdown(markdown, options = {}) {
	...
	try {
		...
		if (!Array.isArray(mappings) || mappings.length === 0) throw new Error('Invalid mapping');
	} catch {
	...
	for (let line of lines) {
		...
		if (trimmed.startsWith('```')) {
			if (!inCodeBlock) {
				...
				// Close code block
				inCodeBlock = false;
				output += `<pre><code${codeLang ? ` class=\"language-${codeLang}\"` : ''}>${codeBuffer.join('\n')}</code></pre>\n`;
				codeBuffer = [];
				codeLang = '';
				continue;
			}
